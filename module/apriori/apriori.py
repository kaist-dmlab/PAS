...
def returnItemsWithMinSupport(itemSet, transactionList, timestampList, itemTimestampIndexDict, dateStampSet, minSupport, freqSet, tidToGranularityTid, granularityTimestampList):
       ...
       of the itemSet each of whose elements satisfies the minimum support"""
        ...
        for itemIdx, item in enumerate(itemSet):
                ...
                for i, element in enumerate(item):
                        elementTsSet = itemTimestampIndexDict[element] # Count element in same time
                        # print "hi" + str(elementTsSet)
                        elementTsSet = set([tidToGranularityTid[ts] for ts in elementTsSet]) # Count element in same granularity
                        # print ">>" + str(elementTsSet)
                        if i == 0:
                            tsSet = elementTsSet
                        else:
                ...
                for ts in tsSet:
                        ...
                        timestamp = granularityTimestampList[ts].strftime('%Y-%m-%d')
...
def getFeatureFromItemset(itemset):
    feature_set = set()
    for item in itemset:
...
def joinSet(itemSet, length, topFeaturesSet, mustContainTime=False):
                ...
                for i in itemSet:
                    for element in i:
                        if 'time_daily' in element:
                            for j in itemSet:
                                candidate = i.union(j)
                                if len(candidate) == length and len(getFeatureFromItemset(candidate)) == length:
                                    if len(topFeaturesSet) > 0:
                                        if len(getFeatureFromItemset(candidate) & (topFeaturesSet)) > 0:
                                            joinedItemSet.add(candidate)
                                    else:
                                        # print candidate
                                        # print getFeatureFromItemset(candidate)
                                        joinedItemSet.add(candidate)
                            break
...
def isImplyImpossible(left, right):
    ...
    relatedMobileConnectionSet = set(['mobile_conn_state[cat]', 'screen_power[cat]'])
    ...
    for left_feature in left_feature_set:
        ...
        if left_feature in relatedBatterySet:
            relatedFeatureSet = relatedBatterySet
            impossible_right_feature_set |= relatedFeatureSet
        if left_feature in relatedWifiSet:
            relatedFeatureSet = relatedWifiSet
            impossible_right_feature_set |= relatedFeatureSet
        if left_feature in relatedMobileConnectionSet:
            relatedFeatureSet = relatedMobileConnectionSet
            impossible_right_feature_set |= relatedFeatureSet
...
def getItemSetTransactionList(data_iterator, granularity_min):
    ...
    for ts, record in enumerate(data_iterator):
        ...
        granularityTimestamp = pd.DatetimeIndex(((np.round(pd.DatetimeIndex([timestamp]).asi8 / (1e9 * 60 * granularity_min))) * 1e9 * 60 * granularity_min).astype(np.int64))[0]
        granularityTimestampList.add(granularityTimestamp)
    ...
    return itemSet, transactionList, timestampList, itemTimestampIndexDict, granularityTimestampList


def runApriori(data_iter, minSupport, minConfidence, tidToGranularityTid, granularityMin, maxK=99999999, removeImposibleImply=True, topFeaturesSet=set()):
    """
